 ...
 * Hovering the pile fans the ghosts out (see .card-stack in globals.css).
 */
export function CardStack({
  ...
  return (
    <div className="card-stack relative isolate">
      {Array.from({ length: depth }, (_, layer) => {
        ...
        return (
          <div
            key={layer}
            aria-hidden
            className="card-stack-ghost pointer-events-none absolute inset-0 -z-10 origin-bottom rounded-xl border border-border/60 bg-card shadow-card"
            style={{
              "--stack-dx": `${dx}px`,
              "--stack-dy": `${dy}px`,
              "--stack-rot": `${angle}deg`,
